...
for opt, arg in opts:
    if opt == "-h":
        print(
            "hill_climber.py -m <metadata file location> -c <maximum number of "
...
while gen <= max_gen and restarts <= max_restarts:
    ...
    # Try random mutations until we see a better solutions, or until we exhaust
    # the number of tries.
...
write_to_file(metadata, solution_best.test_suite, "hillclimber")
